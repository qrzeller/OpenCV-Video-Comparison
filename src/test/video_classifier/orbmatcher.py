...
def orb_matcher(scene_files, dico_match):
    progress = 0
    maximum = len(scene_files)

    for base_file in scene_files:
        progress = progress + 1
        for compare_file in scene_files:
            ...
            if utils.get_video_name(base_file) != utils.get_video_name(compare_file):
                ...
                if len(match) > 0:
                    dico_match.setdefault(utils.get_video_name(base_file), []).append(compare_file)
        print("[%s] Current process: %s/%s" % (str(datetime.now().strftime("%d-%m-%Y %H:%M")), str(progress), str(maximum)))
        utils.save_json_match(dico_match)
    return dico_match
